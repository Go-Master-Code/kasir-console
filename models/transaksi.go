...
func CetakLaporanTransaksiSummary(db *gorm.DB) {
	...
	if errs != nil {
		...
		fmt.Println("Laporan transaksi telah dibuat! (laporan_transaksi.pdf)")
	}
...
func CetakLaporanTransaksiSummaryToday(db *gorm.DB) {
	...
	//errs -> untuk cetak report
	errs := pdf.OutputFileAndClose("laporan_transaksi_" + todayString + ".pdf")
	if errs != nil {
		panic(errs)
	} else {
		fmt.Println("Laporan transaksi telah dibuat!" + "(laporan_transaksi_" + todayString + ".pdf)")
	}
